function [Vb,Vbr] = calcVvector(A,th,Vb0,Vr,varargin)
...
c1 = 1;     % wind force coeff
c2 = 1;     % water drag force coeff
...
[Vb_fmincon,fval] = fmincon(@dummyJ,Vb0,[],[],[],[],[],[],@V_eqns2,optn);
Vb = Vb_fmincon;
% Vbr = Vb - Vr;
...
%     Vbr_hat = Vbr/norm(Vbr);
    Vbr_hat = [cos(A); sin(A)];
%     sine_alpha = det([rs';Vwb'])/norm(Vwb);   % don't divide by zero!!
    rs_X_Vwb = det([rs';Vwb']);
    sine_theta = det([-Vbr_hat';rs']);
%     Fs_mag = c1*norm(Vwb)^2*sine_alpha;
