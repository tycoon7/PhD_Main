...
d = zeros(1,length(e));
for j = 2:beam.nrays
%     d(j) = norm(ray(j).seg(end).P(1)-ray(1).seg(end).P(1), ray(j).seg(end).P(3)-ray(1).seg(end).P(3));
    d(j) = norm(beam.ray(j).seg(end).P-beam.ray(1).seg(end).P);
end
RMS_SpotSize = mean(d);
