...
% build beam objects (they have ray objects defined as properties within)
...
% spot diagram perturbation vectors
for i = 1:beam.nrays
    j = i-1;
%     gamma(:,i) = wn(4*i:4*i+3);
    gamma(:,i) = wn(7*j+4:7*j+7);
end

%% exact raytrace of perturbed system
...
% build mirror objects
for s = 1:length(e)
    Mp(s) = MirrorClass(e(s),f(s),Vp(:,s),q(s),psi(:,s),rotz(-Thz(s)));
end

% build beam objects
beamp = BeamClass(BW,rd,P0_chief,I0,Mp);

%% plots
% nominal raytrace plot
f1 = figure(1);
beam.plotNomRaytrace(beam,f1)

% nominal spot diagram with linear perturbed spot diagram
f2 = figure(2);
grid on
BeamClass.plotPertSpots(beam,f2,gamma);
BeamClass.plotNomSpots(beam,f2)

% exact pert spot diagram with linear pert spot diagram
f3 = figure(3);
grid on
BeamClass.plotPertSpots(beam,f3,gamma);
BeamClass.plotNomSpots(beamp,f3)
