...
clc;clear;close all;
I1= 1.5;        % (kg*m^2) MOI, Mirror 1
I2= 1.5;        % (kg*m^2) MOI, Mirror 2
Ib= 5;          % (kg*m^2) MOI, Base
m1= 1;          % (kg) Mass, Mirror 1
m2= 1;          % (kg) Mass, Mirror 2
mb= 50;         % (kg) Mass, Base
h= 4;           % (m) Nominal Vertical Distance, Mirrors-Sensor
hm= 0.1;        % (m) Nominal Vertical Distance, Base-Mirrors
L= 1;           % (m) Horizontal Distance, Center-Mirror CoR and CoM
l= 0.1;         % (m) Radius, Mirror
k= 2.5*10^6;    % (N/m) Modulus of Elasticity, Actuators
c= 100;         % (N/m/s) Damping coefficient, Actuators

%Initial Set up
phi=atan(h/L);      % (rad) Nominal Angle, Mirror CoM-Sensor ref base
al_0=pi/4-phi/2;    % (rad) Nominal Angle, Mirror Plane-Base (1-negative, 2-positive)
...
% relative distance & relative velocity matrix
DDM=[DM zeros(4,6);
     zeros(4,6) DM];
 
% Actuator system matrix
ACM=[k 0 0 0 c 0 0 0;
     0 k 0 0 0 c 0 0;
     0 0 k 0 0 0 c 0;
     0 0 0 k 0 0 0 c];
...
% output matrix (the zeroth derivative terms)
...
% ouput matrix with rates
% C=[Cm zeros(4,6);
%    zeros(4,6) Cm];

% output matrix (no derivative terms, rates)
C = [    Cm    ;
     zeros(4,6)];
